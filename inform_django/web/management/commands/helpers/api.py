...
LogLevel = [logging.DEBUG, logging.INFO, logging.WARNING, logging.ERROR]
...
class Request:
    ...
    def __init__(
        ...
        if log_level not in LogLevel:
            logging.error(f"Invalid log level '{log_level}' provided to Requests class.")
        self.log_level = log_level
        logging.basicConfig(level=self.log_level)

    @classmethod
    def api_get_request(self, page: int) -> Response:
        current_params: Dict[str, Union[str, int]] = self.params.update({"page": page})
    ...
    def handle_pagination(self, max_retries: int) -> List[Json]:
        ...
        while (total_pages is None or page < total_pages):
            ...
            # handle paging    
            if "paging" not in json_reponse:
                logging.error(f"Response to request to '{self._url}{self.endpoint}' does not contain key `paging`.")
            if "total" not in json_reponse.get("paging"):
                logging.error(f"Response to request to '{self._url}{self.endpoint}' does not contain key `paging['total']`.")
            ...
            data_dict[pages] = json_response
            pages += 1
        # combine results into data_list
        data_list: List[Json] = []
        for results in data_dict.values():
            if "response" not in results:
                logging.error(f"Response to request to '{self._url}{self.endpoint}' does not contain key `response`.")
    ...
    def request(self, max_retries: int = 5) -> List[Json]: 
        response_data: List[Json] = handle_pagination()
